...
const { N2Stat } = require('../models/Stat.js');
...
router.route('/export').get(async (req, res) => {
    ...
    try {
        const stats = await N2Stat.find(query).sort({ date: 'asc' });

        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Statistiques N2');
        ...
        stats.forEach(stat => {
            worksheet.addRow({
                ...stat.toObject(),
                date: new Date(stat.date).toLocaleDateString('fr-FR', { timeZone: 'UTC' }),
            });
        });
...
router.route('/:id').delete((req, res) => {
    N2Stat.findByIdAndDelete(req.params.id)
...
router.route('/seed-week').post(async (req, res) => {
    try {
        ...
        for (let i = 0; i < 5; i++) { // Loop for 5 days (Mon-Fri)
            ...
            const promise = N2Stat.findOneAndUpdate(
